...
int main()
{
     ...
     */
     ...
     asm("cpuid" : "=d"(edx), "=b"(ebx),"=c"(ecx) : "a"(0):"cc");
     char id[13];
     strcpy(id,ebx);
     strcat(id,edx);
     strcat(id, ecx);
    ...
    asm("movl $0x80000008, %%eax\n"
        "cpuid\n"
        "shr $8,%%eax\n"
        "and $255,%%eax" : "=eax"(linear_address)::"cc","ebx","ecx","edx");
    

    printf("Number of Linear Address Bits: %u\n", linear_address);

    asm("movl $0x01, %%eax\n"
        "cpuid\n": "=edx"(value),"=ecx"(aes)::"cc","ebx");
    fpu = value & 1;
    acpi = value >> 22;
    acpi = acpi & 1;
    sse = value >> 25;
    ...
    asm("movl $0x01, %%eax\n"
        "cpuid": "=eax"(value):: "cc","ebx","ecx","edx");
    apic = value >> 9;
    apic = apic & 1;

    asm("movl $0x07, %%eax\n"
        "movl $0x00, %%ecx\n"
        "cpuid": "=ebx"(smap),"=ecx"(value)::"cc","edx");
    smap = value >> 20;
    smap = smap & 1;
    vaes = value >> 9;
    vaes = vaes & 1;
